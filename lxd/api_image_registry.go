...
func imageRegistriesPost(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		if req.Protocol == api.ImageRegistryProtocolLXD && !registryPublic {
			exists, err := dbCluster.ClusterLinkExists(ctx, tx.Tx(), registryCluster)
			if err != nil {
				return fmt.Errorf("Failed fetching cluster link %q for image registry: %w", registryCluster, err)
			} else if !exists {
		...
		err = dbCluster.CreateImageRegistryConfig(ctx, tx.Tx(), req.Name, req.Config)
		if err != nil {
...
func imageRegistryPost(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		if err != nil {
			return fmt.Errorf("Failed fetching image registry %q: %w", oldName, err)
		}
...
func updateImageRegistry(s *state.State, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		if existingRegistry.Protocol == api.ImageRegistryProtocolLXD && !registryPublic {
			exists, err := dbCluster.ClusterLinkExists(ctx, tx.Tx(), registryCluster)
			if err != nil {
				return fmt.Errorf("Failed fetching cluster link %q for image registry %q: %w", registryCluster, existingRegistry.Name, err)
			} else if !exists {
				return fmt.Errorf("Cluster link %q does not exist", registryCluster)
			}
		}
		...
		if descriptionChanged {
			err = dbCluster.UpdateImageRegistry(ctx, tx.Tx(), existingRegistry.Name, dbCluster.ImageRegistry{
		...
		err = dbCluster.UpdateImageRegistryConfig(ctx, tx.Tx(), existingRegistry.Name, existingRegistry.Config)
		if err != nil {
