...
func ConnectCluster(ctx context.Context, clusterLink api.ClusterLink, args *lxd.ConnectionArgs) (lxd.InstanceServer, error) {
	...
	return nil, fmt.Errorf("Failed connecting to any address of cluster link %q", clusterLink.Name)
}
...
func RefreshClusterLinkVolatileAddresses(ctx context.Context, s *state.State, name string) error {
	...
	err := s.DB.Cluster.Transaction(ctx, func(ctx context.Context, tx *db.ClusterTx) error {
		...
		clusterLink, err = dbLink.ToAPI(ctx, tx.Tx())
		if err != nil {
			return err
		}
	...
	_, _, err = CheckClusterLinkCertificate(ctx, finalAddresses, shared.CertFingerprint(targetCert), version.UserAgent)
	if err != nil {
