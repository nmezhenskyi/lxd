...
func updateClusterLink(s *state.State, r *http.Request, isPatch bool) response.Response {
	...
	// Reject attempts to change volatile.* keys — these are managed internally only.
	// Re-insert existing volatile values so a PUT that omits them doesn't erase them.
	for k, currentVal := range apiClusterLink.Config {
		if !strings.HasPrefix(k, "volatile.") {
			continue
		}

		userVal, ok := req.Config[k]
		if ok && userVal != currentVal {
			return response.BadRequest(fmt.Errorf("Configuration key %q cannot be set directly", k))
		}
...
func clusterLinkPost(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		if len(identities) == 0 {
			return fmt.Errorf("Identity with ID %d not found", clusterLink.IdentityID)
		}
...
func clusterLinksPost(d *Daemon, r *http.Request) response.Response {
	...
	clusterLinkType := dbCluster.ClusterLinkType(req.Type)
	...
	if req.Name != "" {
		...
		reverter.Add(func() {
			err := s.DB.Cluster.Transaction(s.ShutdownCtx, func(ctx context.Context, tx *db.ClusterTx) error {
				return dbCluster.DeleteIdentity(ctx, tx.Tx(), api.AuthenticationMethodTLS, fingerprint)
			})
			if err != nil {
		...
		for _, address := range trustToken.Addresses {
			...
			_, _, err = client.RawQuery(http.MethodPost, "/1.0/cluster/links", api.ClusterLinkPost{TrustToken: trustToken.String(), Type: req.Type, ClusterLinkPut: clusterLinkPut, ClusterCertificate: string(networkCert.PublicKey())}, "")
			if err != nil {
...
func clusterLinkStateGet(d *Daemon, r *http.Request) response.Response {
	...
	for i, address := range addresses {
		...
		go func(index int, addr string) {
			...
			targetClient, err := cluster.ConnectClusterLinkAddress(r.Context(), s, *clusterLink, addr)
			if err != nil {
				...
				if err != nil {
					logger.Error("Failed querying cluster link member", logger.Ctx{"address": addr, "err": err})
				} else {
