...
func (r *ClusterLinkRow) ToAPI(ctx context.Context, tx *sql.Tx) (*api.ClusterLink, error) {
	config, err := GetClusterLinkConfig(ctx, tx, r.ID)
	if err != nil {
		return nil, err
	}
...
// UpdateClusterLink updates the cluster link matching the given name.
func UpdateClusterLink(ctx context.Context, tx *sql.Tx, name string, object ClusterLinkRow) error {
...
// GetClusterLinkConfig returns the config for the cluster link with the given ID.
func GetClusterLinkConfig(ctx context.Context, tx *sql.Tx, clusterLinkID int64) (map[string]string, error) {
	q := `SELECT key, value FROM cluster_links_config WHERE cluster_link_id=?`
