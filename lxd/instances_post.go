...
func instancesPost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Source.ImageRegistry == "" && req.Source.Server != "" && req.Source.Protocol == api.ImageRegistryProtocolSimpleStreams { //nolint:staticcheck // Using deprecated field for backward compatibility.
		if api.IsTransitionalSimpleStreamsURL(req.Source.Server) { //nolint:staticcheck // Using deprecated field for backward compatibility.
			registryName := api.TransitionalRegistryName(req.Source.Server) //nolint:staticcheck // Using deprecated field for backward compatibility.
			...
			err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
				...
				// Registry does not exist yet — create it.
				...
				err = dbCluster.CreateImageRegistryConfig(ctx, tx.Tx(), registryID, map[string]string{
					"url":    req.Source.Server, //nolint:staticcheck // Using deprecated field for backward compatibility.
					"public": "true",
				})
				if err != nil {
				...
				logger.Info("Created transitional image registry from deprecated Server field", logger.Ctx{"registry": registryName, "url": req.Source.Server}) //nolint:staticcheck // Using deprecated field for backward compatibility.
	...
	// If the type wasn't specified and we have a LXD protocol registry, pre-fetch the image
	...
	if remoteServer != nil && req.Type == "" && imageRegistry.Protocol == "lxd" {
