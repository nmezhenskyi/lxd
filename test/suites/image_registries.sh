test_image_registries_basic() {
  ...
  # Public cluster links do not require clustered mode, so point one at this server's own
  # API address to exercise a public LXD image registry end to end on a standalone server.
  ...
  lxc image registry create test-lxd-public --protocol=lxd --description="Test public LXD" cluster=self-link source_project=default
  ...
  if lxc image registry create test-val --protocol=lxd cluster=foo 2>/dev/null; then
    echo "ERROR: Create LXD without source_project unexpectedly succeeded" >&2
    lxc image registry delete test-val 2>/dev/null || true
    exit 1
  fi

  # LXD with a source url.
  if lxc image registry create test-val --protocol=lxd cluster=foo source_project=default url=https://lxd.example.com 2>/dev/null; then
    echo "ERROR: Create LXD with a source url unexpectedly succeeded" >&2
    lxc image registry delete test-val 2>/dev/null || true
    exit 1
  fi

  # LXD without cluster link.
  if lxc image registry create test-val --protocol=lxd source_project=default 2>/dev/null; then
    echo "ERROR: Create LXD without cluster link unexpectedly succeeded" >&2
    lxc image registry delete test-val 2>/dev/null || true
    exit 1
  fi
