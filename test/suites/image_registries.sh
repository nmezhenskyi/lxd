test_image_registries_basic() {
  ...
  # Phase 1: create a pending public cluster link and capture the fingerprint of the certificate it fetched.
  ...
  # Phase 2: confirm the pending link by echoing back the fingerprint. The server pins the certificate
  # copy it already holds, and the remote address recorded when the pending link was created, so neither
  # the certificate nor the address is resubmitted.
  lxc query --request POST /1.0/cluster/links --data "{\"name\":\"self-link\",\"type\":\"public\",\"fingerprint\":\"${link_fingerprint}\"}" > /dev/null
  ...
  # Name with a colon is rejected.
  if lxc image registry create test:val url=https://example.com 2>/dev/null; then
    echo "ERROR: Create with colon in name unexpectedly succeeded" >&2
    exit 1
  fi

  # Name with a disallowed character is rejected.
  if lxc image registry create "test val" url=https://example.com 2>/dev/null; then
    echo "ERROR: Create with space in name unexpectedly succeeded" >&2
    exit 1
  fi

  # Name that does not start with an alphanumeric character is rejected.
  for invalid_name in -test .test _test; do
    if lxc image registry create -- "${invalid_name}" url=https://example.com 2>/dev/null; then
      echo "ERROR: Create with name '${invalid_name}' (non-alphanumeric first character) unexpectedly succeeded" >&2
      lxc image registry delete -- "${invalid_name}" 2>/dev/null || true
      exit 1
    fi
  done

  # A name starting with an alphanumeric character and otherwise containing only
  # alphanumeric characters, dashes, dots, and underscores is accepted.
  lxc image registry create test.val-1_ok url=https://example.com
  lxc image registry delete test.val-1_ok
