test_image_expiry() {
  ...
  LXD_DIR=${LXD2_DIR} lxc query --request POST /1.0/cluster/links --data "{\"name\":\"img-link\",\"type\":\"public\",\"fingerprint\":\"${link_fingerprint}\"}" > /dev/null
...
test_image_refresh() {
  ...
  link_fingerprint="$(echo "${pending}" | jq --exit-status -r '.fingerprint')"
  lxc_remote query --request POST /1.0/cluster/links --data "{\"name\":\"img-link\",\"type\":\"public\",\"fingerprint\":\"${link_fingerprint}\"}" > /dev/null
...
test_image_cached() {
  ...
  link_fingerprint="$(echo "${pending}" | jq --exit-status -r '.fingerprint')"
  lxc query --request POST /1.0/cluster/links --data "{\"name\":\"l2-link\",\"type\":\"public\",\"fingerprint\":\"${link_fingerprint}\"}" > /dev/null
