	...
	if !c.flagEmpty {
		...
		if d.HasExtension("image_registries") {
			...
			if sourceRemote == remote {
				...
				if err != nil {
					// Only fall back for 404 (registry not found). Propagate other errors
					// such as 403 (permission denied).
					...
					if remoteConfig.Protocol != "simplestreams" || !api.IsTransitionalSimpleStreamsURL(remoteConfig.Addr) {
						return nil, "", fmt.Errorf("Image registry %q not found", iremote)
					}

					req.Source.Server = remoteConfig.Addr //nolint:staticcheck // Using deprecated field for backward compatibility.
					req.Source.Protocol = "simplestreams" //nolint:staticcheck // Using deprecated field for backward compatibility.
				} else {
