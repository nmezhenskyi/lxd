...
func getImgInfo(conf *config.Config, imgRemote string, imageRef string, imgProject string, source *api.InstanceSource) (lxd.ImageServer, *api.Image, error) {
	...
	if conf.Remotes[imgRemote].Protocol == "simplestreams" {
		...
		if ok && imgProject != "" {
			...
			imgRemoteServer = server.UseProject(imgProject)
		}

		// Get the connection info to fetch the currently used project.
		connInfo, err := imgRemoteServer.GetConnectionInfo()
		if err != nil {
			return nil, nil, fmt.Errorf("Failed getting connection information for remote %q", imgRemote)
		}
