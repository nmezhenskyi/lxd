...
func (c *cmdImageCopy) run(cmd *cobra.Command, args []string) error {
	...
	if destinationServer.HasExtension("image_registries") {
		...
		if sourceRemote == "" {
			sourceRemote = c.global.conf.DefaultRemote
		}
	...
	if sourceServer == nil {
		...
		if sourceRemote != destRemote {
			copyArgs.ImageRegistry = remoteName
		}
	}
...
func (c *cmdImageList) renderImages(filters []string, allImages []api.Image, columns []imageColumn) error {
