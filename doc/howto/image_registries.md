...
To add a registry using the `lxd` protocol (another LXD server), you link to it through a {ref}`cluster link <howto-cluster-links-manage>`.
Use a public cluster link for a public LXD server, or an authenticated cluster link for a private one:
...
To add an image registry, send a `POST` request to the `/1.0/image-registries` endpoint.
    ...
    lxc query --request POST /1.0/image-registries --data '{"name": "my-registry", "protocol": "lxd", "config": {"cluster": "my-cluster-link", "source_project": "default"}}'
